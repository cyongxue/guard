...
int main(int argc, char **argv) {
	...
	if (xml_instance.apply_variables() != 0) {
		error(xml::module.c_str(), "OS_ReadXML error: Applying variables: %s", xml_instance.err().c_str());
		return 1;
	}

//	xml_instance.print_all();

	xml::Node xml_node(NODE_INVALID);
	auto nodes = xml_instance.get_element_by_node(xml_node);
	if (nodes.size() == 0) {
		error(xml::module.c_str(), "OS_GetElementsbyNode error: %s, line: %d", xml_instance.err().c_str(), xml_instance.err_line());
		return 1;
	}

	for (auto it = nodes.begin(); it != nodes.end(); ) {

		info(xml::module.c_str(), "Element: %s -> %s", it->_element.c_str(), it->_content.c_str());
		if ((it->_attributes.size() != 0) && (it->_values.size() != 0)) {
			int k = 0;
			while (k < it->_attributes.size()) {
				info(xml::module.c_str(), "attr %s:%s", it->_attributes[k].c_str(), it->_values[k].c_str());
				k++;
			}
		}
